...
> 
> AMP (Agent Memory Protocol) 是一款完全自主研发、颠覆行业的记忆管理引擎。我们首创了跨应用共享插件机制和图向量混合检索架构，致力于成为**AI记忆管理领域的最顶级形态**。

---

## 🌟 颠覆性原生创新

AMP 不仅仅是一个简单的 K-V 数据库，它是大模型时代的“记忆操作系统”，具备以下引领行业标准的原生核心特性：

### 1. 跨应用共享与隐式捕获 (The "Memory Hub" Extension)
传统的记忆系统受限于单一对话框（如单独的 ChatGPT 或 Cursor），而 AMP 提供原生的 **Chrome 浏览器插件 (`extension/`)** 支持。
- **无感记忆收集**：插件（基于 Manifest V3）在后台通过 `content.js` 隐式捕获用户高亮内容、阅读偏好及高频搜索，将其转化为动态权重的记忆碎片，可以在 `popup.html` 实时查看捕获的全局意图。
- **全局打通 (MCP 革命)**：通过最新引入的 **Model Context Protocol (MCP)** 服务（位于 `mcp/` 目录），无论是网页版大模型、本地终端 Agent，还是代码编辑器（如 Cursor / Claude Desktop），都可以无缝接入同一个 AMP 记忆中枢，实现跨设备、跨生态的上下文共享。

### 2. OS 级的大模型记忆自治 (Autonomous LLM Paging)
我们在业界首创了类操作系统的“内存分页机制”，我们不仅提供了 API，还直接暴露出自研的适配 OpenAI/Claude 的 `getMemoryTools()` 函数。
...
### 🐍 2. Python 核心库 (`/python`)
为 AI Native 开发者及数据科学家量身定制。完美集成 Pydantic 类型校验，且已通过 `setup.py` 标准化打包，可直接发布至 PyPI。
```python
...
### 🌐 3. MCP 记忆服务器 (`/mcp`)
颠覆性特性！基于 Anthropic 的 MCP 标准，让所有兼容 MCP 的客户端直接连通 AMP 的全局记忆网络。
只需运行 `mcp/index.ts`，即可在你的 Claude Desktop 或 IDE 中呼出 `amp_store_memory` 与 `amp_retrieve_memory` 工具。

### 🧩 4. Chrome 浏览器插件锚点 (`/extension`)
提供精美的 `popup.html` 面板与基于 DOM 监听的 `content.js` 采集器。无需编写代码即可部署的全局记忆收集器，自动提取交互意图。详见 `extension/src/background.js` 中的隐式记忆处理引擎。
