...
import { MemoryEvent, MemoryQuery, MemoryResult, MemoryRef, MemoryScope } from './types.js';
...
export interface FileStorageProviderOptions {
  filePath?: string;
  encryptionKey?: string; // 32-byte，以 hex (64 字符) 或 base64 (44 字符) 形式提供
}
...
export class FileStorageProvider implements IStorageProvider {
   ...
   */
  private decrypt(stored: string): unknown {
    if (!this.encryptionKey) return JSON.parse(stored);
  ...
  async retrieve(query: MemoryQuery): Promise<MemoryResult[]> {
    ...
    for (const [id, record] of this.storeMap.entries()) {
      ...
      if (query.minImportance != null && record.metadata.importance < query.minImportance) continue;

      // Fix 4: 作用域隔离。仅对显式声明的维度做相等匹配；未声明的维度不限制。
      if (query.scope) {
        const memScope: MemoryScope = record.scope ?? {};
        if (query.scope.userId !== undefined && memScope.userId !== query.scope.userId) continue;
        if (query.scope.sessionId !== undefined && memScope.sessionId !== query.scope.sessionId) continue;
        if (query.scope.agentId !== undefined && memScope.agentId !== query.scope.agentId) continue;
      }
      ...
      if (score > 0) {
        ...
        if (prevLastAccess !== now) dirty = true;
        ...
        (results[results.length - 1] as MemoryResult & { _decayedImportance?: number })._decayedImportance = decayedImportance;
      }
    }
...
export class RedisStorageProvider implements IStorageProvider {
  ...
  async store(event: MemoryEvent): Promise<MemoryRef> {
    ...
    await this.client.json.set(key, '$', memoryRecord as any);
    return { id, tier: event.tier, created_at: now, updated_at: now };
  }
  ...
  async retrieve(query: MemoryQuery): Promise<MemoryResult[]> {
    ...
    try {
      ...
      for (const doc of searchResult.documents) {
        ...
        if (doc.value && typeof doc.value === 'object' && (doc.value as any).$) {
          record = JSON.parse((doc.value as any).$ as string) as MemoryResult;
        } else if (doc.value && typeof doc.value === 'object') {
          record = doc.value as unknown as MemoryResult;
        } else {
          continue;
        }
        ...
        if (score > 0) {
          ...
          (result as MemoryResult & { _decayedImportance?: number })._decayedImportance = decayedImportance;
          results.push(result);
        }
      }
