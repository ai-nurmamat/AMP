...
// 避免并行测试 / 重复运行之间互相污染。
function makeTmpFile(prefix) {
    return path.join(os.tmpdir(), `amp_test_${prefix}_${Date.now()}_${Math.random().toString(36).slice(2)}.json`);
}
...
describe('AMPCore audit logging', () => {
    ...
    it('should write audit entries for store/retrieve/update/delete', async () => {
        ...
        const storeEntry = entries.find((e) => e.action === 'store');
...
describe('AMPCore with RedisStorageProvider', () => {
    ...
    beforeAll(async () => {
        if (!REDIS_URL)
            return; // itIfRedis 会跳过所有用例，beforeAll 仍会执行
